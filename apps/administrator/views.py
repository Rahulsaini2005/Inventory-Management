...
from apps.location.models import Area, Store
...
class AdminDashboardView(AdminRequiredMixin, LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        items_count  = Item.objects.filter().count()
        products_count  = Product.objects.filter().count()
        areas_count  = Area.objects.filter().count()
        stores_count  = Store.objects.filter().count()
        context = {
